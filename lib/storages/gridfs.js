...
class GridFsStorage {
  ...
  getStream(fileMeta) {
    ...
    const store = new GridStore(this.db, new ObjectID(id.toString()), "r", {
      root: "fs"
    });
  ...
  putFile(path, options) {
    ...
    options = rest || {};
    const gridStore = Bb.promisifyAll(
      new GridStore(this.db, new ObjectID(), fileName, "w", options)
    );

    return Bb.try(() => gridStore.openAsync())
  ...
  replaceFile(fileMeta, path, options) {
    ...
    return Bb.bind(this)
      .then(() => {
        options = rest || {};
        options.root = "fs";
        const store = Bb.promisifyAll(
          new GridStore(this.db, id, fileName, "w", options)
        );
